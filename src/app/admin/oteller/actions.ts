...
const roomSchema = z.object({
  id: z.string().uuid().optional(), // existing room id
...
const hotelFormSchema = z.object({
  ...
  // Amenities — dinamik olarak okunacak
  // Rooms — JSON string olarak gelir
...
export async function saveHotelAction(
  ...
  // Junction: deal_categories (önce sil, yeniden yaz)
  ...
  // dealId at this point is guaranteed set (insert above)
  if (!dealId) {
  ...
  // Oda tipleri: gelen ID'leri kalanlardan sil + upsert
  ...
  redirect(`/admin/oteller/${dealId}?saved=1`);
}
